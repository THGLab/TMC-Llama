...
def get_basic_mol(xyz_file, overall_charge, huckel=True):
...
def get_metal_charge_from_smiles(tmc_smiles):
    """Extract the transition metal formal charge (oxidation state) from a
    TMC-SMILES string.

    Useful when you have a reference TMC-SMILES (e.g. from a database) and
    want to assign the same metal oxidation state when converting a .xyz
    to TMC-SMILES via get_tmc_mol(..., metal_charge=...).

    Args:
        tmc_smiles (str): A TMC-SMILES string containing exactly one
            transition metal (the first metal found is used).

    Returns:
        int: The formal charge on the transition metal atom, or None if
        the SMILES could not be parsed or no transition metal was found.
    """
    mol = Chem.MolFromSmiles(tmc_smiles)
    if mol is None:
        return None
...
def get_tmc_mol(xyz_file, overall_charge, with_stereo=False, metal_charge=None, use_huckel_AC=False):
    """Get TMC mol object from given xyz file.

    Args:
        xyz_file (str): Path to TMC xyz file
        overall_charge (int): Overall charge of TMC (used to infer metal charge
            when metal_charge is not provided).
        with_stereo (bool): Whether to percieve stereochemistry from the 3D data
        metal_charge (int, optional): If set, use this as the transition metal
            formal charge (oxidation state) instead of inferring it from
            overall_charge - sum(ligand charges). Useful when you have a
            reference TMC-SMILES with known metal oxidation state. The resulting
            molecule total charge will be metal_charge + sum(ligand charges).

    Returns:
        tmc_mol (rdkit.Chem.rdchem.Mol): TMC mol object
    """
    if metal_charge is not None:
        ## we have metal oxidation state so we don't have overall charge
        ## huckel is deactivated
        mol = get_basic_mol(xyz_file, overall_charge, huckel=False)
    else:
        ## we don't have metal oxidation state so we have options
        if use_huckel_AC:
            mol = get_basic_mol(xyz_file, overall_charge, huckel=True)
        else:
            mol = get_basic_mol(xyz_file, overall_charge, huckel=False)
    ...
    tm_ox = (
        metal_charge
        if metal_charge is not None
        else overall_charge - total_lig_charge
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional: use this as the transition metal oxidation state (formal charge) instead of inferring from --charge",
    )
    parser.add_argument(
        "--log_level",
        type=str,
    ...
    tmc_mol = get_tmc_mol(
        args.xyz_file, args.charge, with_stereo=False, metal_charge=args.metal_charge
    )
